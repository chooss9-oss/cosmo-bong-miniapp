    ...
    await Promise.allSettled(
      ...
      batch.map(async (url) => {
        ...
        try {
          ...
          // ВРЕМЕННО ОТКАЧЕНО: текстовый поиск "Нет в наличии" по всему
          // body страницы ловил не только сам товар, а что-то ещё на
          // странице (сайдбар/виджет), из-за чего почти весь каталог
          // ошибочно помечался как "нет в наличии". Возвращаем прежнее,
          // безопасное поведение — фолбэк применяется только если
          // селекторы выше вообще ничего не нашли — пока не будет найден
          // точный, узкий селектор именно блока наличия товара.
